...
void get_options(const int argc, char **argv, int *nx, int *ny, int *ndump, float *left, float *right, float *top, float *bottom)
{
  int c;
...
void set_borders(float *grid, int nx, int ny, float left, float right, float top, float bottom)
{
  int i, j;
  for (i = 0; i < nx + 2; i++)
  {
...
void init_grids(float *grid, float *grid_new, int nx, int ny, float left, float right, float top, float bottom)
{
...
void dump_grid(float *grid, int nx, int ny)
{
  int i, j, k;
  set_filename();
