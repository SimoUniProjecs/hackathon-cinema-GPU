...
!  Parallel initialisation
  ...
  ndev = acc_get_num_devices( acc_get_device_type() )
  call acc_set_device_num(mod(rank, ndev), acc_get_device_type())
...
!  Do a boundary swap of psi
  !$acc data copy(psi(0:m+1, 0:ln+1)) create(psitmp)
...
!  Do a boundary swap
     call nvtxstartrange("haloswap",1)
     call haloswap(psi, m, ln, comm, rank, size)
     call nvtxendrange()
